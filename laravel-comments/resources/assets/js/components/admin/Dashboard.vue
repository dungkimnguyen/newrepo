...
export default {
  ...
  methods: {
     ...
     * @return {Number}
     */
    commentCount ({ page_id, commentable_id, commentable_type }) {
      if (page_id) {
        return this.pageCount[page_id] || 0
      }

      return this.commentableCount[`${commentable_id}|${commentable_type}`] || 0
    },
     ...
     * @return {Number}
     */
    commentPage ({ page_id, commentable_id, commentable_type }) {
      if (page_id) {
        return `?page_id=${page_id}`
      }

      return `?commentable_id=${commentable_id}&commentable_type=${commentable_type}`
    },
